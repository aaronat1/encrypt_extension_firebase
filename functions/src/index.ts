...
function encrypt(plaintext: string): string {
  ...
  // Store as: iv(12) + authTag(16) + ciphertext — all base64
...
async function handleWrite(
  ...
  for (const field of FIELDS_TO_ENCRYPT) {
    ...
    // Skip if already encrypted (avoid loops)
    if (data[encField] !== undefined && data[field] === undefined) continue;

    if (typeof data[field] === "string") {
      try {
        updates[encField] = encrypt(data[field] as string);
  ...
  functions.logger.info("Encrypting fields", {
    path: after.ref.path,
...
// Firestore triggers
export const encryptFieldsL1 = functions.firestore
  .document("{c1}/{d1}")
  .onWrite((change) => handleWrite(change));

export const encryptFieldsL2 = functions.firestore
  .document("{c1}/{d1}/{c2}/{d2}")
  .onWrite((change) => handleWrite(change));

export const encryptFieldsL3 = functions.firestore
  .document("{c1}/{d1}/{c2}/{d2}/{c3}/{d3}")
  .onWrite((change) => handleWrite(change));

export const encryptFieldsL4 = functions.firestore
  .document("{c1}/{d1}/{c2}/{d2}/{c3}/{d3}/{c4}/{d4}")
  .onWrite((change) => handleWrite(change));

export const encryptFieldsL5 = functions.firestore
  .document("{c1}/{d1}/{c2}/{d2}/{c3}/{d3}/{c4}/{d4}/{c5}/{d5}")
  .onWrite((change) => handleWrite(change));

// HTTPS decryption endpoint — server-side use only
export const decryptField = functions.https.onRequest(async (req, res) => {
  if (req.method !== "POST") {
    res.status(405).json({ error: "Method not allowed. Use POST." });
    return;
  }

  const { documentPath, field } = req.body as { documentPath?: string; field?: string };

  if (!documentPath || !field) {
    res.status(400).json({ error: "Missing required body fields: documentPath, field" });
    return;
  }
  ...
  try {
    ...
    if (typeof data[encField] !== "string") {
      res.status(404).json({ error: `Encrypted field "${encField}" not found or not a string` });
      return;
    }

    const plaintext = decrypt(data[encField] as string);
